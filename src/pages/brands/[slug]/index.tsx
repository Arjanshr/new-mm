...
import { resetBrandPage } from "@/redux/slice/brandSlice";
import { useAppDispatch, useAppSelector } from "@/redux/store";
import {
  getBrandProducts,
  getCategoryForBrand,
} from "@/redux/thunks/brandThunk";
...
const BrandProducts: React.FC<BrandProductProps> = ({ brand }) => {
  ...
  const brandData = useAppSelector((state: any) => state.brandState);
  const { categories, products, loading, total } = brandData;

  useEffect(() => {
    if (brand) {
      dispatch(getCategoryForBrand({ id: brand.id }));
    }
  }, [brand, dispatch]);
  ...
  }, [brand, dispatch, page, pageSize, selectedCategories, minPrice, maxPrice]);
...
export async function getServerSideProps(context: any) {
  try {
    ...
    const brand = await axios.get(`${API_BASE_URL}/brands/${slug}`);
    return {
      props: {
        brand: brand.data.data || null,
        title: `${brand?.data?.data?.name} | ${SITE_NAME}`,
        url: `${SITE_URL}/brands/${slug}`,
        image: `${WEBSITE_BASE_URL}/${brand.data?.data?.imageLink}`,
      },
    };
  } catch (error) {
    return {
